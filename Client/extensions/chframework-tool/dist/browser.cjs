...
async function readWorkbookFromFile(file) {
  ...
  return readSync(buf, { type: "buffer", cellDates: false });
}
async function convertExcelDir(excelDir, jsonOutDir, options) {
  const compact = (options == null ? void 0 : options.compact) !== false;
  const files = (await fs.promises.readdir(excelDir)).filter((f) => f.toLowerCase().endsWith(".xlsx") && !f.startsWith("~$"));
  await fs.promises.mkdir(jsonOutDir, { recursive: true });
  const written = [];
  for (const f of files) {
    ...
    try {
      ...
      console.warn(`[excel2json] 跳过，无法读取文件: ${full} -> ${(e == null ? void 0 : e.message) || e}`);
      continue;
    }
  }
  return written;
...
function pascalCase(name) {
  return name.split(/[^A-Za-z0-9]+/g).filter(Boolean).map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join("");
}
function mapTsType(t) {
  return (t || "").trim().toLowerCase() === "number" ? "number" : "string";
}
async function readHeaderFromXlsx(file) {
  await fs.promises.access(file, fs.constants.R_OK);
  const buf = await fs.promises.readFile(file);
  const wb = readSync(buf, { type: "buffer", cellDates: false });
  const ws = wb.Sheets[wb.SheetNames[0]];
  if (!ws) return null;
  const rows = utils.sheet_to_json(ws, { header: 1, raw: true });
  if (!rows || rows.length < 3) return null;
  const types = (rows[1] || []).map((v) => v == null ? "" : String(v).trim().toLowerCase());
  const keys2 = (rows[2] || []).map((v) => v == null ? "" : String(v).trim());
  if (!keys2.some((k) => k.toLowerCase() === "id")) return null;
  return { types, keys: keys2 };
}
function genInterfaceCode(iface, types, keys2) {
  const fields = [];
...
async function generateModels(excelDir, outFile, options) {
  const importPath = (options == null ? void 0 : options.importPath) ?? "../../CHFramework/Framework";
  const files = (await fs.promises.readdir(excelDir)).filter((f) => f.toLowerCase().endsWith(".xlsx") && !f.startsWith("~$")).map((f) => path.join(excelDir, f)).sort((a, b) => path.parse(a).name.localeCompare(path.parse(b).name, "en", { sensitivity: "base" }));
  const pieces = [];
  for (const f of files) {
    try {
      ...
      console.warn(`[excel2model] 跳过（无法读取）: ${f} -> ${(e == null ? void 0 : e.message) || e}`);
    }
  }
  if (!pieces.length) {
...
const methods = {
  async open_excel_tool() {
    Editor.Panel.open(`${_package.name}.excelToolPanel`);
  },
  async open_mvvm_generator() {
    Editor.Panel.open(`${_package.name}.mvvmGeneratorPanel`);
  },
  async convertExcel(options) {
    const { excelDir, jsonOutDir, modelOutDir, FrameworkTS } = options || {};
    ...
    try {
      ...
      console.error("[excel-tool] excel2json 失败:", (e == null ? void 0 : e.message) || e);
      return false;
    }
    try {
      ...
      console.error("[excel-tool] excel2model 失败:", (e == null ? void 0 : e.message) || e);
      return false;
    }
    try {
      ...
      console.warn("[excel-tool] 刷新资源失败:", (e == null ? void 0 : e.message) || e);
    }
    return true;
  }
};
async function load() {
  console.log(`load ${_package.name}`);
}
function unload() {
  console.log(`unload ${_package.name}`);
}
exports.load = load;
